...
std::string HashTableDictionary::csvStats(
    ...
    const std::string &compaction_state) const
{
    std::ostringstream oss;

    // prepend the extra harness fields
    oss << impl << "," << profile << "," << trace_path << "," << N << "," << elapsed_ms << ",";

    // hash table statistics
    oss << (numInserts + numDeletes + numLookups) << ","      // ops_total
        << numInserts << ","                                  // inserts
        << numDeletes << ","                                  // erases
        << TABLE_SIZE << ","                                  // table_size
        << numberOfActive << ","                              // active
        << (TABLE_SIZE - numberOfActive - numberOfTombstones) << ","  // available
        << numberOfTombstones << ","                          // tombstones
        << totalProbes << ","                                 // total_probes
        << numInserts << ","                                   // inserts again for CSV
        << numDeletes << ","                                   // deletes again
        << numLookups << ","                                   // lookups
        << numFullScans << ","                                 // full_scans
        << numCompactions << ","                                // compactions
        << maxValuesInTable << ","                              // max_in_table
        << static_cast<int>((TABLE_SIZE - numberOfActive - numberOfTombstones) * 100.0 / TABLE_SIZE) << "," // available_pct
        << static_cast<int>(numberOfActive * 100.0 / TABLE_SIZE) << "," // load_factor_pct
        << static_cast<int>((numberOfActive + numberOfTombstones) * 100.0 / TABLE_SIZE) << "," // eff_load_factor_pct
        << static_cast<int>(numberOfTombstones * 100.0 / TABLE_SIZE) << "," // tombstones_pct
        << static_cast<double>(totalProbes) / (numInserts + numDeletes + numLookups) << "," // average_probes
        << probe_type << ","                                    // probe_type string
        << compaction_state;                                    // compaction_on/off
